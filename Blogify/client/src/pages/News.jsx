...
import { fetchAllNewsArticles, formatImageUrl } from "../utils/api";
...
const News = () => {
  ...
  const [activeCategory] = useState("news"); // Always "news" for this page
    ...
    const fetchPosts = async () => {
      try {
        ...
        if (response.success) {
          setPosts(
            response.data.map((article) => {
              return {
                ...
                tags: article.tags,
                category: "news",
                subcategory: article.tags?.[0] || "general",
                imageUrl: formatImageUrl(article.imageUrl),
                date: (() => {
                  try {
          ...
          toast.error(response.message || "Failed to fetch news articles");
        }
      } catch (err) {
        console.error("Error fetching news articles:", err);
        setError("Failed to fetch news articles");
        toast.error("Failed to fetch news articles. Please try again later.");
      } finally {
        setLoading(false);
      }
    };
  ...
  const filterPosts = () => {
    // Filter by category (always "news" for this page)
    let filtered = posts;
  ...
  const getSubcategoryTitle = () => {
    if (activeSubcategory === "all") return "All News";
    return `${
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-amber-100 to-white py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center mb-8">
          <MotionDiv
            ...
            transition={{ duration: 0.5 }}
            className="inline-flex items-center justify-center p-2 bg-amber-100 rounded-full mb-4"
          >
          ...
          >
            <span className="text-amber-600">{getSubcategoryTitle()}</span>
          </MotionH1>
          <MotionP
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
          ...
          >
            Stay updated with the latest news and developments from around the
            world.
          </MotionP>
        </div>
