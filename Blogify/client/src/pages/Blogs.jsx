...
import { fetchAllBlogPosts, formatImageUrl } from "../utils/api";
...
const Blogs = () => {
  ...
  const [activeCategory] = useState("blog"); // Always "blog" for this page
          ...
          setPosts(
            response.data.map((blog) => {
              return {
                ...
                imageUrl: formatImageUrl(blog.imageUrl),
                date: (() => {
                  try {
  ...
  const filterPosts = () => {
    // Filter by category (always "blog" for this page)
    let filtered = posts;
  ...
  const getSubcategoryTitle = () => {
    if (activeSubcategory === "all") return "All Blog Posts";
    return `${
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-indigo-100 to-white py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center mb-8">
          ...
          >
            <span className="text-indigo-600">{getSubcategoryTitle()}</span>
          </MotionH1>
          <MotionP
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
          ...
          >
            Discover insightful articles, stories, and news from our talented
            writers.
          </MotionP>
        </div>
